    ...
    async def tag(self, ctx):
        ...
        if args[1].upper() == "CREATE":
            if get_profile(ctx.author.id)[1] >= 30 or "Moderator" in [role.name for role in ctx.message.author.roles] or "Manager" in [role.name for role in ctx.message.author.roles]:
                ...
                tagName = args[2]
                try:
                    sql.execute_query("ibm.db", "INSERT INTO tags (name, content, submittedBy) VALUES ('%s', '%s', '%s')" % (tagName, tagContent, tagSubmitter))
            ...
            if get_profile(ctx.author.id)[1] >= 30 or "Moderator" in [role.name for role in ctx.message.author.roles] or "Manager" in [role.name for role in ctx.message.author.roles]:
                ...
                tagName = args[2]
                try:
                    sql.execute_query("ibm.db", "INSERT INTO tags (name, content, submittedBy, isImage) VALUES ('%s', '%s', '%s', 1)" % (tagName, tagContent, tagSubmitter))
                    await ctx.send("Tag successfully created")
                except:
                    await ctx.send("A tag already exists with that name")
            else:
            ...
            if "Moderator" in [role.name for role in ctx.message.author.roles] or "Manager" in [role.name for role in ctx.message.author.roles]:
                tagName = args[2]
                tag = sql.db_query("ibm.db", "SELECT name FROM tags WHERE name = '%s'" % (tagName))
                if len(tag) == 0: # tag does not exists
                    await ctx.send("There is no tag with that name")
                else:
                    sql.execute_query("ibm.db", "DELETE FROM tags WHERE name = '%s'" % (tagName))
                    await ctx.send("Tag has been successfully deleted")
        else:
            tagName = args[1]
            tag = sql.db_query("ibm.db", "SELECT name, content, submittedBy, isImage FROM tags WHERE name = '%s'" % (tagName))
            if not len(tag) == 0:
                if tag[0][3] == 1:
