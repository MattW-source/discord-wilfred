...
def generate_leaderboard_string(guild, page=1):
    leaderboard_2d_array = sql.db_query("ibm.db", "SELECT UserID, Balance FROM Members WHERE NOT UserID = 1 ORDER BY Balance DESC")
...
async def leaderboard_main(ctx, page=1, leaderboard_message_object=None, self=None):
    ...
    embed.set_author(name="Baltop (Page %s)" % (str(page)))
